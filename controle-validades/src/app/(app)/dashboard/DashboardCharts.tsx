    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-3 flex flex-col h-[210px]">
        <div className="flex items-center justify-between mb-2 shrink-0">
          <h3 className="font-bold text-slate-800 text-sm">Evolução Diária (Últimos 7 dias)</h3>
          <select className="text-[10px] border-slate-200 rounded-lg text-slate-600 focus:ring-primary bg-slate-50 py-0.5 px-1.5">
        ...
        <div className="flex-1 w-full min-h-0">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={evolutionData} margin={{ top: 5, right: 10, left: -25, bottom: 0 }}>
              <defs>
              ...
              />
              <Legend verticalAlign="top" height={20} iconType="circle" wrapperStyle={{ fontSize: '10px' }}/>
      ...
      {/* Bar Chart */}
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-3 flex flex-col h-[210px]">
        <h3 className="font-bold text-slate-800 text-sm mb-2 shrink-0">Top Perdas por Departamento</h3>
        <div className="flex-1 w-full min-h-0">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={deptoData} layout="vertical" margin={{ top: 0, right: 10, left: -10, bottom: 0 }}>
              <XAxis type="number" hide />
              <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#64748b' }} />
              <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}/>
              <Bar dataKey="perdas" fill="#f97316" radius={[0, 4, 4, 0]} barSize={16} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
